...
async def data_certificates():
    ret = {}
    certificates = await store_data()
    for i in range(1, certificates.Count+1):
        ret[f'certificate_{i}'] = await certificate_info(certificates.Item(i))
...
@app.post('/signer')
async def signer_file(file: UploadFile = File(...), pin: Optional[str] = None, serial_number: Optional[str] = None):
    if pin:
        signer = await signature_data_pin(pin, serial_number)
    else:
        signer = await signature_data(serial_number)
...
async def unsigner_file(file: UploadFile = File(...)):
    ...
    return JSONResponse(content={
        'unsignedContent': _signedData.Content,
